...
class RotatedAnchorGenerator(nn.Module):
    ...
    The module support computing anchors at multiple sizes, aspect ratios and
    angles per feature map. This module assumes aspect ratio = height / width
    for each anchor.

    sizes, aspect_ratios and angles should have the same number of elements, and
    it should correspond to the number of feature maps.

    sizes[i], aspect_ratios[i] and angles[i] can have an arbitrary number of
    elements, and RotatedAnchorGenerator will output a set of sizes[i] *
    aspect_ratios[i] * angles[i] anchors per spatial location for feature map i.
    ...
    def __init__(
        ...
        if not isinstance(aspect_ratios[0], (list, tuple)):
            aspect_ratios = (aspect_ratios,) * len(sizes)
        if not isinstance(angles[0], (list, tuple)):
            angles = (angles,) * len(sizes)

        assert len(sizes) == len(aspect_ratios) == len(angles)
    ...
    def num_anchors_per_location(self):
        return [len(s) * len(a) * len(ang) for s, a, ang in zip(self.sizes, self.aspect_ratios, self.angles)]
    ...
    def grid_anchors(self, grid_sizes: List[List[int]], strides: List[List[Tensor]]) -> List[Tensor]:
        ...
        for size, stride, base_anchors in zip(grid_sizes, strides, cell_anchors):
            ...
            # shifts_x = torch.arange(offset * stride_width, grid_width * stride_width, step=stride_width, dtype=torch.int32, device=device)
            # shifts_y = torch.arange(offset * stride_height, grid_height * stride_height, step=stride_width, dtype=torch.int32, device=device)
            shifts_x = torch.arange(0, grid_width, dtype=torch.int32, device=device) * stride_width
            shifts_y = torch.arange(0, grid_height, dtype=torch.int32, device=device) * stride_height
            shift_y, shift_x = torch.meshgrid(shifts_y, shifts_x, indexing="ij")
            shift_x = shift_x.reshape(-1)
